...
import { exchangeConfluenceCode } from "~/backend/oauth.server";
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  try {
    await exchangeConfluenceCode(code, state);
    ...
    console.error("Confluence OAuth Error:", error?.response?.data || error?.message || error);
