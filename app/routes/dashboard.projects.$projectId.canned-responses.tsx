...
import { ChevronLeft, Plus, Trash2, MessageSquare, Pencil, Check, X } from "lucide-react";
              ...
              {editingId === cr.id ? (
                <editFetcher.Form
                  ...
                  onSubmit={(e) => {
                    e.preventDefault();
                    editFetcher.submit(
                      { _action: "edit", id: cr.id, title: editTitle, body: editBody },
                      { method: "post" }
                    );
                    setEditingId(null);
                  }}
                ...
                >
                  <input
                    ...
                    required
                    className="w-full px-4 py-2.5 border border-brand-border rounded-xl outline-none focus:border-primary transition-colors text-sm font-bold"
                  />
                  <textarea
                    value={editBody}
                    ...
                    required
                    rows={4}
                    className="w-full px-4 py-2.5 border border-brand-border rounded-xl outline-none focus:border-primary transition-colors text-sm resize-none"
                  />
                  <div className="flex gap-2 justify-end">
                    <button
                      type="button"
                      onClick={() => setEditingId(null)}
                      className="p-2 rounded-xl text-zinc-400 hover:bg-zinc-50 transition-colors"
                    >
                      <X className="w-4 h-4" />
                    </button>
                    <button
                      type="submit"
                      className="p-2 rounded-xl text-white bg-primary hover:scale-105 transition-all"
                    >
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
                    <button
                      type="button"
                      onClick={() => { setEditingId(cr.id); setEditTitle(cr.title); setEditBody(cr.body); }}
                      className="p-2 rounded-xl text-zinc-300 hover:text-primary hover:bg-primary/5 transition-all cursor-pointer"
                    >
                    ...
                    <deleteFetcher.Form method="post">
                      ...
                      <button
                        type="submit"
                        className="p-2 rounded-xl text-zinc-300 hover:text-red-400 hover:bg-red-50 transition-all cursor-pointer"
                      >
